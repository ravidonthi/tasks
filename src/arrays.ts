...
export function countShortWords(words: string[]): number {
    return words.reduce(
        (total: number, word: string) => total + (word.length < 4 ? 1 : 0),
        0,
    );
}
...
export function injectPositive(values: number[]): number[] {
    ...
    const previousNum = index === -1 ? values : values.slice(0, index);
    const previousNumSum = previousNum.reduce(
        (total: number, num: number) => total + num,
        0,
    );

    return index === -1 ?
            [...values, previousNumSum]
        :   [
